...
class ReadExcl(object):
    ...
    def create_data(self):
        ...
        tb_1 = []
        # case_type = "点源文件上传-通用表-其他工业-规上企业-通用表_p07_原辅料及产品信息表，"
        # case_type = "点源文件上传-通用表_p05_窑炉信息表，"
        case_type = "点源文件上传-通用表_p07_原辅料及产品信息表，"
        case_type1_1 = "{}验证【{}】行业的所属排放源填写非可选项{}报错校验"
        case_type1 = "{}验证【{}】行业的所属排放源可选项为{}"
        case_type2 = "{}验证【{}】行业【{}】所属排放源的产品类型可选项为{}"
        case_type2_1 = "{}验证【{}】行业【{}】所属排放源的产品类型填写非可选项{}报错校验"
        case_type3 = "{}验证【{}】行业【{}】所属排放源【{}】产品类型的工段/工艺名称可选项为{}"
        case_type3_1 = "{}验证【{}】行业【{}】所属排放源【{}】产品类型的工段/工艺名称填写非可选项{}报错校验"
        case_type4 = "{}验证【{}】行业【{}】所属排放源【{}】产品类型【{}】工段/工艺名称的单位可选项为{}"
        case_type4_1 = "{}验证【{}】行业【{}】所属排放源【{}】产品类型【{}】工段/工艺名称的单位填写非可选项{}报错校验"

        cases = []
        for n in range(minrow + 1, maxrow + 1):
            tb_1.append(self.sheet.cell(n, 2).value)

        for m in set(tb_1):
            tb_2 = []  # 存放所属排放源
            for n in range(minrow+1, maxrow+1):
                if self.sheet.cell(n, 2).value == m:
                    tb_2.append(self.sheet.cell(n, 3).value)
            ...
            for i in set(tb_2):
                ...
                for t in range(minrow + 1, maxrow + 1):
                    if self.sheet.cell(t, 3).value == i:
                        tb_3.append(self.sheet.cell(t, 4).value)
                # print(set(tb_3))
            ...
            for i in set(tb_2):
                ...
                for a in set(tb_3):
                    ...
                    for b in range(minrow + 1, maxrow + 1):
                        if self.sheet.cell(b, 4).value == a:
                            tb_4.append(self.sheet.cell(b, 5).value)
                    # print(set(tb_4))
                ...
                for a in set(tb_3):
                    ...
                    for c in set(tb_4):
                        ...
                        for d in range(minrow + 1, maxrow + 1):
                            if self.sheet.cell(d, 5).value == c:
                                tb_5.append(self.sheet.cell(d, 6).value)
...
if __name__ == '__main__':
    ...
    print(data_path)
    r1 = ReadExcl(data_path, '通用表原辅料及产品')
    # r1 = ReadExcl(data_path, '通用表窑炉')
    cases = r1.create_data()
    r2 = ReadExcl(case_path, '通用表_p07_原辅料及产品信息表')
    r2.open()
